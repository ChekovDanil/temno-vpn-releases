...
Игровой режим доступен только в Windows. На Android, iOS и iPadOS он не входит
в продукт; для macOS это лишь возможный отдельный будущий этап, которого нет в
текущем handoff.
...
python scripts/validate_catalog.py --live
```
...
- временные URL с query-параметрами, смешение тегов и тег, не совпадающий с
  платформой и версией;
- включение автообновления до внедрения проверки подписей каталога и файла;
- расхождение SHA-256 и файлов `checksums/*.txt`;
- расхождение главного и платформенных каталогов.

Проверка с `--live` дополнительно сверяет существование GitHub Release,
состояние prerelease, имена, размеры и доступные GitHub SHA-256 всех объявленных
asset. Её запускают перед публикацией изменения каталога, поскольку она требует
доступа к GitHub API.
